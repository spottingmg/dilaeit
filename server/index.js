...
app.listen(port, '0.0.0.0', () => {
    ...
    // Selbst-Ping alle 10 Min damit Render nicht schläft
    const selfUrl = process.env.RENDER_EXTERNAL_URL || `http://localhost:${port}`;
    setInterval(async () => {
        try {
            await fetch(`${selfUrl}/api/health`, { signal: AbortSignal.timeout(5000) });
            console.log(`[Uptime] Ping OK`);
        } catch (e) { console.warn(`[Uptime] Ping fehlgeschlagen:`, e.message); }
    }, 10 * 60 * 1000); // alle 10 Min
});
